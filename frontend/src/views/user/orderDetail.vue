<template>
  <div class="order-detail-page" v-loading="loading">
    <div class="container" v-if="orderInfo.id">
      ...
      <!-- 物流信息 -->
      <div class="card logistics-card" v-if="logisticsTraceList.length > 0">
        <div class="card-title">物流信息</div>
        <div class="logistics-body">
          <el-icon><Van /></el-icon>
          <span>物流信息</span>
          <span class="logistics-num">运单号：{{ logisticsId || '暂无' }}</span>
        </div>

        <div class="trace-toggle">
          <el-button type="text" @click="showTrace = !showTrace">
            {{ showTrace ? '收起物流轨迹' : '查看物流轨迹' }}
            <el-icon :class="showTrace ? 'rotate-up' : ''"><ArrowDown /></el-icon>
          </el-button>
        </div>

        <el-collapse-transition>
          <div v-if="showTrace" class="trace-timeline">
            <div 
              class="trace-item" 
              v-for="(trace, index) in logisticsTraceList" 
              :key="trace.id"
            ...
            >
              ...
              <div class="trace-content">
                <div class="trace-text">
                  {{ trace.content }}
                  <span v-if="index === 0" class="current-tag"></span>
                </div>
                <div class="trace-time">{{ formatTime(trace.createTime) }}</div>
              </div>
            </div>
          </div>
        </el-collapse-transition>
...
import { CopyDocument, Van, ArrowDown } from '@element-plus/icons-vue'
import { 
  getOrderDetailService, 
  cancelOrderService, 
  confirmOrderService 
} from '@/api/user/order.js'
import { getLogisticsListService ,signLogisticsService} from '@/api/user/logistics.js'
...
// 获取物流轨迹
const fetchLogisticsTrace = async (orderId) => {
  try {
    const res = await getLogisticsListService(orderId)
    logisticsTraceList.value = (res.data || []).sort((a, b) => 
      new Date(b.createTime) - new Date(a.createTime)
    )
    if (logisticsTraceList.value.length > 0) {
      logisticsId.value = logisticsTraceList.value[0].logisticsId
    }
  } catch (e) {
...
// 自动滚动到物流模块
const scrollToLogistics = async () => {
  await nextTick()
  const el = document.querySelector('.logistics-card')
  if (el) {
    el.scrollIntoView({
      behavior: 'smooth',
      block: 'start'
...
onMounted(async () => {
  const id = route.query.orderId || route.params.id
  if (!id) {
    ElMessage.error('缺少订单标识')
  ...
  // 如果来自查看物流 → 自动展开 + 自动滚动
  if (route.query.showLogistics === 'true') {
...
const fetchDetail = async (id) => {
  ...
  loading.value = true
  try {
    const res = await getOrderDetailService(id)
    orderInfo.value = res.data || {}
    await fetchLogisticsTrace(id)
  } catch (err) {
...
const handleCancel = async () => {
  try {
    await ElMessageBox.confirm('确定取消该订单？取消后无法恢复', '提示', { type: 'warning' })
    await cancelOrderService(orderInfo.value.id)
    ElMessage.success('订单已取消')
    fetchDetail(orderInfo.value.id)
  } catch (e) {
...
const handleConfirm = async () => {
  try {
    ...
    await confirmOrderService(orderInfo.value.id)
    await signLogisticsService(orderInfo.value.id)
...
const toAfterSale = () => {
  ...
  if (!firstItem) {
    ElMessage.warning('商品异常，无法申请售后')
    return
  }
  router.push({
    path: '/afterSaleApply',
    query: { orderId: orderInfo.value.id, item: JSON.stringify(firstItem) }
  })
}
...
const toComment = () => {
  ...
  if (!firstItem?.productId) {
    ElMessage.warning('商品信息异常，无法评价')
    return
  }
  router.push({
    path: '/commentPage',
    query: { orderId: orderInfo.value.id, productId: firstItem.productId }
  })
}

const handlePay = () => {
  router.push({ path: '/pay', query: { orderId: orderInfo.value.id, amount: orderInfo.value.payAmount } })
}
</script>
